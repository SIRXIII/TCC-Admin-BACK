...
class PartnerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('profileImage')) {
            $path = $request->file('profileImage')->store('partners/profile', 'public');
            $partner->update(['profile_photo' => $path]);
        }

        if ($request->hasFile('license_front')) {

            $path = $request->file('license_front')->store('partners/licenses', 'public');
        ...
        if ($request->hasFile('license_back')) {
            ...
            $path = $request->file('license_back')->store('partners/licenses', 'public');
            PartnerDocument::create([
                'partner_id' => $partner->id,
                'type'       => 'license',
                'side'       =>  'back',
        ...
        if ($request->hasFile('ownerId_front')) {
            ...
            $path = $request->file('ownerId_front')->store('partners/owner_ids', 'public');
            PartnerDocument::create([
                'partner_id' => $partner->id,
                'type'       => 'owner_id',
                'side'       => 'front',
        ...
        if ($request->hasFile('ownerId_back')) {
            $path = $request->file('ownerId_back')->store('partners/owner_ids', 'public');
            PartnerDocument::create([
                'partner_id' => $partner->id,
                'type'       => 'owner_id',
                'side'       => 'back',
